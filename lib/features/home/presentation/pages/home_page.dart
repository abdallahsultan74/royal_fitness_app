...
import '../../../challenges/presentation/pages/challenge_details_page.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  List<ChallengeTemplate>? _templates;

  @override
  void initState() {
    super.initState();
    _progress.fetchChallengeTemplates().then((list) {
      if (mounted) setState(() => _templates = list);
    });
  }

  ChallengeTemplate? _bannerTemplate(ChallengeProgress? active) {
    if (active != null) return active.toTemplate();
    final c = _templates;
    if (c == null || c.isEmpty) return null;
    for (final t in c) {
      if (t.slug == 'royal-transform-beginner') return t;
    }
    return c.first;
  }

  String? _challengeImageUrl(ChallengeProgress? active, ChallengeTemplate? tpl) {
    final u = active?.coverImageUrl ?? tpl?.coverImageUrl;
  ...
  void _openChallenge(
    ...
    if (tpl == null) {
      MainShellScope.goToTab(context, 2);
      return;
    }
    Navigator.of(context).push<void>(
      MaterialPageRoute<void>(
        builder: (_) => ChallengeDetailsPage(template: tpl, active: active),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RoyalTabScaffold(
      child: StreamBuilder<DailyStat>(
        ...
        builder: (context, snapStat) {
          final stat = snapStat.data ??
              DailyStat(
          ...
          return StreamBuilder<UserProfile>(
            ...
            builder: (context, snapProf) {
              ...
              return StreamBuilder<ChallengeProgress?>(
                stream: _progress.watchActiveChallenge(),
                builder: (context, snapCh) {
                  final rawActive = snapCh.data;
                  final active = coachFeatureEnabled(profile, 'challenges', defaultWhenAllowed: true)
                      ? rawActive
                      : null;
                  final tpl = _bannerTemplate(active);
                  final stepsDisplay = HomeActivityGoals.effectiveSteps(stat);
                  final outer = HomeActivityGoals.ringCalories(stat);
                  final mid = HomeActivityGoals.ringMinutes(stat);
                  final inner = HomeActivityGoals.ringSteps(stat);
                  final centerPct =
                      (HomeActivityGoals.overallProgress(stat) * 100).round();
                  final bmiVal = profile?.bmi;
                  final bmiText = bmiVal != null ? bmiVal.toStringAsFixed(1) : '--';
                  final markerFrac = homeBmiMarkerFraction(bmiVal);
                  final imgUrl = _challengeImageUrl(active, tpl) ?? _kFallbackChallengeImg;
                  final titleChallenge = tpl != null
                      ? tpl.displayTitle(context.locale.languageCode)
                      : 'home_challenge_title'.tr();
                  final dayLine = active != null
                      ? 'challenge_progress_day'.tr(
                          args: [
                            '${active.currentDay}',
                            '${active.daysCount}',
                          ],
                  ...
                  final prog = active != null
                      ? (active.progressPercent / 100).clamp(0.0, 1.0)
                      : 0.0;
                  final progLabel =
                      '${(active?.progressPercent ?? 0).clamp(0, 100).round()}%';
                    ...
                    children: [
                        ...
                        child: Material(
                          ...
                          child: InkWell(
                            onTap: () {
                              Navigator.of(context).push<void>(
                                MaterialPageRoute<void>(
                                  builder: (_) => const DailyGoalDetailPage(),
                                ),
                              );
                            },
                            child: RoyalGlassPanel(
                        ...
                        child: Material(
                          ...
                          child: InkWell(
                            onTap: () {
                              Navigator.of(context).push<void>(
                                MaterialPageRoute<void>(
                                  builder: (_) => BmiDetailPage(profile: profile),
                                ),
                              );
                            },
                            child: RoyalGlassPanel(
                                          ...
                                          children: [
                                            ...
                                            Flexible(
                                              child: Container(
                                                ...
                                                child: Text(
                                                  homeBmiStatusLabel(profile?.bmiStatus),
                                                  maxLines: 1,
                                                  overflow: TextOverflow.ellipsis,
                                                  style: TextStyle(
                                                    color: _bmiBadgeColor(profile?.bmiStatus),
                                                    fontSize: 11,
                                                  ),
                                                ),
                                              ),
                      ...
                      Padding(
                        ...
                        child: Material(
                          ...
                          clipBehavior: Clip.antiAlias,
                          child: InkWell(
                            onTap: () => _openChallenge(context, active, tpl),
                            child: ClipRRect(
                              borderRadius: BorderRadius.circular(24),
                              child: Stack(
                                children: [
                ...
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          const Icon(Icons.timer, size: 10, color: AppColors.creamDim),
                          Text(
                            '${minutes}m',
                            style: const TextStyle(
                              color: AppColors.creamDim,
